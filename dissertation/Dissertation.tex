...
\newcommand{\vertex}{\node[vertex]}
\usepackage{relsize}
\usepackage[unicode,bookmarks=true,bookmarksopen=true,bookmarksnumbered=true,colorlinks=true,linkcolor=blue,citecolor=blue,urlcolor=blue,pagebackref=false]{hyperref}
\usepackage{bookmark}
...
\renewcommand{\theFancyVerbLine}{\LR{\arabic{FancyVerbLine}}}
...
\LARGE{\textbf{صوری‌سازی ترجمه حذف وابستگی در سیستم‌های نوع محض لایه‌ای}}
\end{center}
\vfill
\begin{center}
نگارش\\
...
\includegraphics[scale=0.13]{signature.png}
...
\input{chekide-temp}\addcontentsline{toc}{chapter}{چكيده}

\input{pishgoftar-temp}\addcontentsline{toc}{chapter}{پيشگفتار}
...
\part{مفاهيم و تعاريف مقدماتی}
\input{S1-temp}
\part{سیستم‌های نوع محض لایه‌ای و ترجمه حذف وابستگی}
\input{S2-temp}
\input{S3-temp}
...
% \input{vaje_fars-eng-temp}

\begin{latin}
\begin{abstract}
In this thesis, the dependency-erasure translation introduced by
\lr{Nathan Mull}
for \lr{Tiered Pure Type Systems} \cite{mull2023}
is fully formalized and mechanically verified in the proof assistant
\lr{Coq}.
The translation functions, together with all intermediate lemmas—including the substitution lemma, preservation of reduction paths, and lemmas related to injectivity and image—are proved in a fully machine-checked manner.
The main theorem of \lr{Mull}, stating that weak normalization implies strong normalization for all tiered pure type systems with specific rules, is mechanically verified for the first time.

As a result, an open and extensible library is developed, providing a solid foundation for extending this translation to more expressive dependent systems such as the \lr{Calculus of Constructions}.
The results of this work constitute a significant step toward the proof of the
\lr{BGK} conjecture\footnote{\lr{Barendregt–Geuvers–Klop}} \cite{geuvers1993}
in dependent type systems.\\
\textbf{Keywords: } type theory,
tiered pure type systems,
dependency-erasure translation,
...
\begin{center}
\LARGE{Formalization of the Dependency-Eliminating Translation in Tiered Pure Type Systems}
\end{center}
...
\begin{center}
\textbf{Supervisor\\} Majid Alizadeh
\end{center}
