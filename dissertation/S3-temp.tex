...
ترجمه در سطح بالا، استفاده‌های وابسته از قاعده تشکیل نوع تابع در 
\ref{pts_rules}
...
می‌رسیم. سپس ترجمه
$\gamma$
را برای عبارات تعریف می‌کنیم، به طوری که برای هر زمینه
$\Gamma$
و عبارات
$M$ و $A$
که
...
$\Gamma^*$
طوری یافت شود که
$\Gamma^* \vdash_{\lambda S^*} \gamma(M) : \rho_0(A)$.
...
\end{equation*}
باشد؛ و کامل است، هرگاه به ازای هر
$(s_i,s_j) \in \mathcal{R}_{\lambda S}$،
این سیستم
$(j,i)$-کامل باشد.
\end{defn}
...
برای یک
$i$
ثابت، با استفاده از استقرا روی استنتاج نوع پیش می‌رویم.
...
\end{equation*}
در نهایت با استفاده از قاعده معرفی متغیر داریم
\begin{equation*}
...
اگر
$j \ge i + 2$،
از روشی مشابه آنچه در حالت معرفی متغیر در بالا ذکر شد، استفاده می‌کنیم؛ یعنی برای هر
$k$
که
$i \le k \le j - 1$
...
\end{equation*}
سپس با استفاده مکرر از قاعده تشکیل نوع تابع به همراه
\begin{equation*}
...
مواردی که در آن‌ها آخرین گام استنتاج، معرفی متغیر یا تضعیف است، مشابه موارد متناظر در لم
...
\end{equation*}
بدین ترتیب، با استفاده مکرر از قاعده تشکیل نوع تابع، که همگی با استفاده از ترجمه‌های از پیش تعریف‌شده صورت می‌گیرد؛
\begin{equation*}
...
از این‌ رو، با ترکیب این نتیجه با نتیجه‌ی
\cite{barthe2001}،
